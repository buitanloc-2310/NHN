...
async function openAccountAccess(){
  try{
    ...
    modal(`<button onclick="closeModal()">Đóng</button>
      <h2>Chưa thể kiểm tra trạng thái tài khoản</h2>
      <p class="muted">Hệ thống không đọc được trạng thái khởi tạo quản trị từ cơ sở dữ liệu. Không chuyển sang màn hình đăng nhập để tránh gây hiểu nhầm rằng đã có tài khoản.</p>
      <div class="notice bad">${fmt(err?.message||"SETUP_STATUS_ERROR")}</div>
      <button class="primary" onclick="closeModal();openAccountAccess()">Thử lại</button>`);
  }
}
