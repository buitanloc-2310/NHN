...
async function home(){
  ...
  const heroText=String(cfg.hero_text||"Một không gian số dành cho hoạt động, bảng tin, hồ sơ và xác thực Giấy chứng nhận của Nhà Hán Ngữ.").trim();
  const heroCover=String(cfg.hero_cover_url||"/assets/nhn-logo.jpg").trim()||"/assets/nhn-logo.jpg";
  const featureCards=Array.isArray(cfg.home_feature_cards)?cfg.home_feature_cards:[];
  const serviceCards=Array.isArray(cfg.home_service_cards)?cfg.home_service_cards:[];
  const cardsHtml=(items)=>items.map(x=>`<a class="card portal-card" href="${E(x.href||'#home')}"><h3>${E(x.title||'')}</h3><p>${E(x.text||'')}</p><span>Khám phá →</span></a>`).join("");

  app.innerHTML=`<section class="hero">
    <div>
      <div class="eyebrow">${E(cfg.product_eyebrow||"ỨNG DỤNG NHÀ HÁN NGỮ")}</div>
      <h1>${E(heroTitle)}</h1>
      <p>${E(heroText)}</p>
      <div class="actions">
  ...
  <section class="section intro-band">
    <div class="section-heading"><span class="pill">NHN APP</span><h2>${E(cfg.home_intro_title||"")}</h2></div>
    <p class="lead">${E(cfg.home_intro_text||"")}</p>
    <div class="grid">${cardsHtml(featureCards)}</div>
  </section>

  <section class="section soft-section">
    <div class="split-copy">
      <div><span class="eyebrow">GIỚI THIỆU</span><h2>${E(cfg.home_about_title||"")}</h2></div>
      <p>${E(cfg.home_about_text||"")}</p>
    </div>
  </section>

  <section class="section">
    <div class="section-heading"><span class="pill">TIỆN ÍCH</span><h2>${E(cfg.home_services_title||"Dịch vụ và tiện ích")}</h2></div>
    <div class="grid">${cardsHtml(serviceCards)}</div>
  </section>

  <section class="section">
    <div class="section-heading"><span class="pill">BẢNG TIN</span><h2>Bảng tin mới</h2></div>
    <div class="grid">${n.items.slice(0,6).map(card).join("")||'<div class="card">Chưa có bản tin.</div>'}</div>
  </section>

  <section class="section join-panel">
    <div><span class="eyebrow">CỘNG ĐỒNG</span><h2>${E(cfg.home_join_title||"")}</h2><p>${E(cfg.home_join_text||"")}</p></div>
    <div class="actions"><a class="btn" href="#participate">Tham gia Nhà Hán Ngữ</a><a class="btn secondary" href="#lookup">Tra cứu & xác thực</a></div>
  </section>`;
}
...
function adminNav(){
  return `<aside class="admin-nav">
    ...
    <a href="#admin/settings">Nội dung & giao diện</a>
  </aside>`;
}
...
async function settingsPage(){
  ...
  const field=(key,label,type="text")=>{
    ...
    return `<div class="field"><label>${E(label)}</label>${type==="textarea"?`<textarea name="${E(key)}" rows="5">${E(text)}</textarea>`:`<input name="${E(key)}" value="${E(text)}">`}</div>`;
  };
  adminPage("Nội dung & giao diện",`
    <div class="notice"><b>Không phá dữ liệu cũ.</b> Trang này chỉ cập nhật các khóa nội dung/giao diện. Bản tin, sự kiện, hồ sơ, tài khoản, GCN/GXN và dữ liệu nghiệp vụ vẫn nằm nguyên trong các module hiện có.</div>
    <form id="settingsForm">
      <div class="card settings-section"><h2>Nhận diện ứng dụng</h2>
        ${field("product_name","Tên sản phẩm")}
        ${field("product_eyebrow","Nhãn đầu trang")}
        ${field("brand_slogan","Slogan")}
        ${field("hero_title","Tiêu đề Hero")}
        ${field("hero_text","Mô tả Hero","textarea")}
        ${field("hero_cover_url","Ảnh Hero / đường dẫn")}
      </div>
      <div class="card settings-section"><h2>Trang chủ</h2>
        ${field("home_intro_title","Tiêu đề giới thiệu")}
        ${field("home_intro_text","Nội dung giới thiệu","textarea")}
        ${field("home_feature_cards","Các thẻ nổi bật (JSON)","textarea")}
        ${field("home_about_title","Tiêu đề về Nhà Hán Ngữ")}
        ${field("home_about_text","Nội dung về Nhà Hán Ngữ","textarea")}
        ${field("home_services_title","Tiêu đề dịch vụ")}
        ${field("home_service_cards","Các thẻ dịch vụ (JSON)","textarea")}
        ${field("home_join_title","Tiêu đề cộng đồng")}
        ${field("home_join_text","Nội dung cộng đồng","textarea")}
      </div>
      <div class="card settings-section"><h2>Menu & chân trang</h2>
        ${field("nav_items","Menu chính (JSON)","textarea")}
        ${field("footer_about_title","Tiêu đề footer")}
        ${field("footer_about_text","Giới thiệu footer","textarea")}
        ${field("footer_parent_text","Đơn vị chủ quản","textarea")}
        ${field("footer_quick_links","Liên kết footer (JSON)","textarea")}
        ${field("footer_transparency_title","Tiêu đề liên thông")}
        ${field("footer_transparency_text","Nội dung liên thông","textarea")}
        ${field("footer_legal_text","Ghi chú cuối trang","textarea")}
      </div>
      <div class="card settings-section"><h2>Liên hệ & hệ thống</h2>
        ${field("website","Website chính")}
        ${field("receiver_email","Email tiếp nhận")}
        ${field("support_email","Email hỗ trợ")}
        ${field("hotline","Hotline")}
        ${field("certificate_api_path","Đường dẫn API GCN/GXN")}
      </div>
      <button type="submit">Lưu toàn bộ nội dung</button>
    </form>`);

  settingsForm.onsubmit=async e=>{
    e.preventDefault();
    const fd=new FormData(e.target),items={};
    const jsonKeys=new Set(["home_feature_cards","home_service_cards","nav_items","footer_quick_links"]);
    for(const [k,v] of fd.entries()){
      if(jsonKeys.has(k)){
        try{items[k]=JSON.parse(String(v||"[]"))}catch{alert(`JSON không hợp lệ ở ${k}`);return}
      }else items[k]=String(v??"");
    }
    await api("/api/admin/settings",{method:"PUT",body:{items}});
    state.config=await api("/api/config").catch(()=>state.config||{});
    hydrateChrome();
    toast("Đã lưu nội dung & giao diện");
    settingsPage();
  };
}
...
function hydrateChrome(){
  ...
  if(brandName) brandName.textContent=cfg.app_name||"NHÀ HÁN NGỮ";
  if(brandSlogan) brandSlogan.textContent=cfg.slogan||"Kết nối tri thức • Mở lối tương lai";
  document.title=cfg.product_name||"Ứng dụng Nhà Hán Ngữ";

  const nav=document.getElementById("mainNav");
  if(nav && Array.isArray(cfg.nav_items) && cfg.nav_items.length){
    nav.innerHTML=cfg.nav_items.map(x=>`<a href="${E(x.href||'#home')}"${String(x.href||'').startsWith('http')?' target="_blank" rel="noopener noreferrer"':''}>${E(x.label||'Liên kết')}</a>`).join("");
  }

  const setText=(id,value)=>{const el=document.getElementById(id);if(el)el.textContent=value||""};
  setText("footerAboutTitle",cfg.footer_about_title||cfg.product_name||"Ứng dụng Nhà Hán Ngữ");
  setText("footerAboutText",cfg.footer_about_text||"");
  setText("footerParentText",cfg.footer_parent_text||"");
  setText("footerTransparencyTitle",cfg.footer_transparency_title||"Xác thực & liên thông");
  setText("footerTransparencyText",cfg.footer_transparency_text||"");
  setText("footerLegal",cfg.footer_legal_text||"");
  const contact=document.getElementById("footerContact");
  if(contact) contact.textContent=[cfg.receiver_email,cfg.hotline,cfg.website].filter(Boolean).join(" · ");
  const links=document.getElementById("footerLinks");
  if(links && Array.isArray(cfg.footer_quick_links)){
    links.innerHTML=cfg.footer_quick_links.map(x=>`<a href="${E(x.href||'#home')}"${String(x.href||'').startsWith('http')?' target="_blank" rel="noopener noreferrer"':''}>${E(x.label||'Liên kết')}</a>`).join("");
  }
}
