...
async function createSupportTicket(env,submission){
  const answers=JSON.parse(submission.answers_json||"{}"),code=await nextCode(env,"NHN-TICKET"),id=uid("ticket");
...
export async function publicRoute(request,env,url){
  ...
  if(p==="/api/config"&&request.method==="GET"){
    ...
    return json({
      app_name:await getSetting(env,"app_name","Nhà Hán Ngữ"),
      app_short_name:await getSetting(env,"app_short_name","Nhà Hán Ngữ"),
      app_url:env.APP_URL||await getSetting(env,"app_url",""),
      website:await getSetting(env,"website","https://nhahanngu.io.vn"),
      hotline:await getSetting(env,"hotline",""),
      receiver_email:await getSetting(env,"receiver_email","nhahanngu.vn@gmail.com"),
      slogan:await getSetting(env,"brand_slogan",""),
      product_name:await getSetting(env,"product_name","Ứng dụng Nhà Hán Ngữ"),
      product_eyebrow:await getSetting(env,"product_eyebrow","ỨNG DỤNG NHÀ HÁN NGỮ"),
      hero_title:await getSetting(env,"hero_title","Kết nối tri thức. Mở lối tương lai."),
      hero_text:await getSetting(env,"hero_text","Một không gian số dành cho hoạt động, bảng tin, hồ sơ và xác thực Giấy chứng nhận của Nhà Hán Ngữ."),
      hero_cover_url:await getSetting(env,"hero_cover_url","/assets/nhn-logo.jpg"),
      home_intro_title:await getSetting(env,"home_intro_title","Một không gian số thống nhất cho cộng đồng Nhà Hán Ngữ"),
      home_intro_text:await getSetting(env,"home_intro_text","Ứng dụng Nhà Hán Ngữ kết nối các dịch vụ và hoạt động trên một nền tảng thống nhất."),
      home_feature_cards:await getSetting(env,"home_feature_cards",[]),
      home_about_title:await getSetting(env,"home_about_title","Nhà Hán Ngữ trong hệ sinh thái Sky First"),
      home_about_text:await getSetting(env,"home_about_text",""),
      home_services_title:await getSetting(env,"home_services_title","Dịch vụ và tiện ích"),
      home_service_cards:await getSetting(env,"home_service_cards",[]),
      home_join_title:await getSetting(env,"home_join_title","Cùng học, cùng chia sẻ, cùng phát triển"),
      home_join_text:await getSetting(env,"home_join_text",""),
      nav_items:await getSetting(env,"nav_items",[]),
      footer_about_title:await getSetting(env,"footer_about_title","Ứng dụng Nhà Hán Ngữ"),
      footer_about_text:await getSetting(env,"footer_about_text",""),
      footer_parent_text:await getSetting(env,"footer_parent_text",""),
      footer_quick_links:await getSetting(env,"footer_quick_links",[]),
      footer_transparency_title:await getSetting(env,"footer_transparency_title","Xác thực & liên thông"),
      footer_transparency_text:await getSetting(env,"footer_transparency_text",""),
      footer_legal_text:await getSetting(env,"footer_legal_text",""),
      certificate_api_path:await getSetting(env,"certificate_api_path","/api/lookup/certificate?code="),
  ...
  if(submitMatch&&request.method==="POST"){
    ...
    const receiver=row.recipient_email||await getSetting(env,"receiver_email","nhahanngu.vn@gmail.com");
  ...
  if(p==="/api/lookup/submission"&&request.method==="GET"){
    ...
    if(!code||!email) return json({error:"MISSING_FIELDS",message:"Vui lòng nhập đầy đủ mã hồ sơ và email đã đăng ký."},400);
    ...
    if(!row) return json({error:"NOT_FOUND",message:"Không tìm thấy hồ sơ phù hợp với mã và email đã nhập."},404);
  ...
  if(p==="/api/lookup/certificate"&&request.method==="GET"){
    const code=String(url.searchParams.get("code")||"").trim();
    if(!code) return json({error:"MISSING_FIELDS",message:"Vui lòng nhập mã GCN/GXN cần xác thực."},400);

    const row=await env.DB.prepare(`
      SELECT code,cert_type,full_name,content,status,issued_at,metadata_json
      FROM certificates
      WHERE code=? AND status IN ('issued','revoked')
      LIMIT 1
    `).bind(code).first();

    if(!row) return json({error:"NOT_FOUND",message:"Không tìm thấy GCN/GXN tương ứng với mã đã nhập."},404);

    let metadata={};
    try{metadata=row.metadata_json?JSON.parse(row.metadata_json):{}}catch{metadata={}}

    const statusText={issued:"Còn hiệu lực",revoked:"Đã thu hồi"}[row.status]||row.status;
