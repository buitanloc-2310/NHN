...
export async function adminRoute(request,env,url){
  ...
  if(rolesMatch&&request.method==="PUT"){
    ...
    if(target.email.toLowerCase()==="nhahanngu.vn@gmail.com" && !(user.roles||[]).some(r=>r.role_id==="super_admin"))
    ...
    for(const old of existing){
      if(old.role_id==="super_admin"&&target.email.toLowerCase()==="nhahanngu.vn@gmail.com"&&!roles.some(r=>(r.role_id||r)==="super_admin"))
        return json({error:"ROOT_SUPER_ADMIN_CANNOT_BE_REMOVED"},403);
    }
    if(target.email.toLowerCase()==="nhahanngu.vn@gmail.com") await env.DB.prepare("DELETE FROM user_roles WHERE user_id=? AND role_id!='super_admin'").bind(targetId).run(); else await env.DB.prepare("DELETE FROM user_roles WHERE user_id=?").bind(targetId).run();
    for(const r0 of roles){
      ...
      if(role==="super_admin"&&target.email.toLowerCase()!=="nhahanngu.vn@gmail.com"&&!(user.roles||[]).some(r=>r.role_id==="super_admin")) continue;
  ...
  if(userResetMatch&&request.method==="POST"){
    ...
    if(!target)return json({error:"NOT_FOUND"},404);
    if(target.email.toLowerCase()==="nhahanngu.vn@gmail.com")return json({error:"ROOT_SUPER_ADMIN_PROTECTED"},403);
  ...
  if(userStatusMatch&&request.method==="PATCH"){
    ...
    if(!target)return json({error:"NOT_FOUND"},404);
    if(target.email.toLowerCase()==="nhahanngu.vn@gmail.com"&&status!=="active") return json({error:"ROOT_SUPER_ADMIN_PROTECTED"},403);
    await env.DB.batch([
