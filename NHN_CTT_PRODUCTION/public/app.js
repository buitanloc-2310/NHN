...
async function home(){
  const [n,cfg,portal]=await Promise.all([
    api("/api/public/news").catch(()=>({items:[]})),
    api("/api/config").catch(()=>({})),
    api("/api/public/portal").catch(()=>({announcements:[],shortcuts:[],stats:{}}))
  ]);
  ...
  ${(portal.announcements||[]).length?`<section class="section" style="padding-bottom:0"><div class="notice good"><b>${E(portal.announcements[0].title)}</b><p>${E(portal.announcements[0].body)}</p></div></section>`:""}

  <section class="section">
    <div class="metric-grid">
      <div class="metric"><b>${fmt(portal.stats?.certificates||0)}</b><span>GCN/GXN trong hệ thống</span></div>
      <div class="metric"><b>${fmt(portal.stats?.events||0)}</b><span>Hoạt động & sự kiện</span></div>
      <div class="metric"><b>${fmt(portal.stats?.news||0)}</b><span>Bản tin công khai</span></div>
      <div class="metric"><b>${fmt(portal.stats?.active_people||0)}</b><span>Nhân sự đang hoạt động</span></div>
    </div>
  </section>

  <section class="section">
    <h2>Dịch vụ nhanh</h2>
    <div class="grid">${(portal.shortcuts||[]).map(x=>`<a class="card" href="${E(x.href)}" style="text-decoration:none;color:inherit"><span class="pill">${E(x.icon||'NHN')}</span><h3>${E(x.title)}</h3><p>${E(x.description||'')}</p></a>`).join('')}</div>
  </section>
...
function adminNav(){
  return `<aside class="admin-nav">
    ...
    <a href="#admin/portal">Trung tâm vận hành</a>
...
async function globalSearch(){
  app.innerHTML=`<section class="section">
    <div class="portal-head"><div><div class="eyebrow">TÌM KIẾM HỢP NHẤT</div><h1>Tìm trên Cổng thông tin</h1><p class="muted">Tìm đồng thời trong bảng tin, hoạt động và văn bản công khai.</p></div></div>
    <div class="portal-search"><input id="portalSearchInput" placeholder="Nhập ít nhất 2 ký tự…" autocomplete="off"><button id="portalSearchBtn">Tìm kiếm</button></div>
    <div id="portalSearchResult" class="section-tight"><div class="card">Nhập từ khóa để bắt đầu.</div></div>
  </section>`;

  const run=async()=>{
    const q=portalSearchInput.value.trim();
    if(q.length<2){portalSearchResult.innerHTML='<div class="notice bad">Vui lòng nhập ít nhất 2 ký tự.</div>';return;}
    portalSearchBtn.disabled=true;portalSearchBtn.textContent='Đang tìm…';
    try{
      const d=await api('/api/public/search?q='+encodeURIComponent(q));
      const labels={news:'Bảng tin',event:'Hoạt động',document:'Văn bản'};
      portalSearchResult.innerHTML=d.items?.length?`<div class="search-results">${d.items.map(x=>`<article class="search-item"><span class="pill">${E(labels[x.type]||x.type)}</span><h3>${fmt(x.title)}</h3><p>${fmt(x.excerpt)}</p></article>`).join('')}</div>`:'<div class="card">Không tìm thấy kết quả phù hợp.</div>';
    }catch(err){portalSearchResult.innerHTML=`<div class="notice bad">${E(err.message)}</div>`}
    finally{portalSearchBtn.disabled=false;portalSearchBtn.textContent='Tìm kiếm';}
  };
  portalSearchBtn.onclick=run;
  portalSearchInput.onkeydown=e=>{if(e.key==='Enter')run()};
}

async function statusPage(){
  app.innerHTML='<section class="section"><h1>Trạng thái hệ thống</h1><div class="card">Đang kiểm tra…</div></section>';
  try{
    const d=await api('/api/status');
    const ok=d.status==='operational';
    const services=Object.entries(d.services||{}).map(([k,v])=>`<div class="status-row"><b>${E(k)}</b><span class="status-dot ${v==='operational'?'ok':'bad'}"></span><span>${E(v)}</span></div>`).join('');
    const incidents=(d.incidents||[]).map(x=>`<article class="card"><span class="pill">${fmt(x.impact)}</span><h3>${fmt(x.title)}</h3><p>${fmt(x.message)}</p><small>${fmt(x.started_at)}</small></article>`).join('');
    app.innerHTML=`<section class="section"><div class="portal-head"><div><div class="eyebrow">NHN SYSTEM STATUS</div><h1>${ok?'Tất cả dịch vụ đang hoạt động':'Một số dịch vụ đang suy giảm'}</h1><p class="muted">Cập nhật ${fmt(d.time)} · ${fmt(d.latency_ms)} ms</p></div><div class="big-status ${ok?'ok':'bad'}">${ok?'OPERATIONAL':'DEGRADED'}</div></div><div class="status-grid">${services}</div><h2>Sự cố đang mở</h2><div class="grid">${incidents||'<div class="card">Không có sự cố đang mở.</div>'}</div></section>`;
  }catch(err){app.innerHTML=`<section class="section"><div class="notice bad"><h2>Không thể kiểm tra trạng thái</h2><p>${E(err.message)}</p></div></section>`}
}

async function portalOps(){
  const d=await api('/api/admin/portal/overview');
  const c=d.counts||{};
  adminPage('Trung tâm vận hành',`
    <div class="metric-grid">
      <div class="metric"><b>${fmt(c.active_users)}</b><span>Tài khoản hoạt động</span></div>
      <div class="metric"><b>${fmt(c.active_people)}</b><span>Nhân sự hoạt động</span></div>
      <div class="metric"><b>${fmt(c.certificates)}</b><span>GCN/GXN</span></div>
      <div class="metric"><b>${fmt(c.events)}</b><span>Hoạt động</span></div>
      <div class="metric"><b>${fmt(c.files)}</b><span>Tệp lưu trữ</span></div>
    </div>
    <div class="grid" style="margin-top:18px">
      <div class="card"><h2>Hồ sơ theo trạng thái</h2>${(d.submissions||[]).map(x=>`<div class="status-row"><span>${fmt(x.status)}</span><b>${fmt(x.count)}</b></div>`).join('')||'<p>Chưa có dữ liệu.</p>'}</div>
      <div class="card"><h2>Ticket hỗ trợ</h2>${(d.tickets||[]).map(x=>`<div class="status-row"><span>${fmt(x.status)}</span><b>${fmt(x.count)}</b></div>`).join('')||'<p>Chưa có dữ liệu.</p>'}</div>
    </div>
    <div class="card" style="margin-top:18px"><h2>Hoạt động hệ thống gần đây</h2>${table(['Tài khoản','Hành động','Đối tượng','Thời gian'],(d.recent_audit||[]).map(x=>`<tr><td>${fmt(x.actor_email)}</td><td>${fmt(x.action)}</td><td>${fmt(x.entity_type)} ${fmt(x.entity_id)}</td><td>${fmt(x.created_at)}</td></tr>`))}</div>
  `);
}
...
async function adminRoute(h){
  ...
  try{
    ...
    if(h==="admin/portal") return portalOps();
...
async function route(){
  ...
  try{
    ...
    if(h==="search") return globalSearch();
    if(h==="status") return statusPage();
...
async function init(){
  ...
  if(state.config.app_name) document.title=`${state.config.app_name} — Cổng Thông tin`;
