...
const errorText=code=>({AUTH_REQUIRED:"Vui lòng đăng nhập để tiếp tục.",INVALID_CREDENTIALS:"Email hoặc mật khẩu không đúng.",EMAIL_NOT_VERIFIED:"Email chưa được xác minh.",ADMIN_ACCESS_NOT_GRANTED:"Tài khoản chưa được cấp quyền quản trị.",SETUP_ALREADY_COMPLETED:"Hệ thống đã có tài khoản quản trị gốc.",RATE_LIMIT:"Bạn thao tác quá nhanh. Vui lòng thử lại sau.",DATABASE_NOT_CONFIGURED:"Cloudflare D1 chưa được gắn vào ứng dụng.",DATABASE_NOT_INITIALIZED:"Cơ sở dữ liệu chưa được khởi tạo. Cần chạy D1 migrations trên production.",REQUEST_FAILED:"Không thể kết nối hệ thống. Vui lòng thử lại."}[code]||"Không thể hoàn tất thao tác. Vui lòng thử lại.");
...
async function api(url,o={}){
  ...
  if(!r.ok){ const e=new Error(errorText(d.error||"REQUEST_FAILED")); e.code=d.error||"REQUEST_FAILED"; throw e; }
  return d;
}
...
async function home(){
  ...
  const heroCover=String(cfg.hero_cover_url||"/assets/nhn-logo.jpg").trim()||"/assets/nhn-logo.jpg";

  app.innerHTML=`<section class="hero">
    <div>
      <div class="eyebrow">ỨNG DỤNG NHÀ HÁN NGỮ</div>
      <h1>${E(heroTitle)}</h1>
      <p>${E(heroText)}</p>
      <div class="actions">
  ...
  <section class="section soft-section">
    <h2>${E(cfg.home_intro_title||"Một ứng dụng thống nhất cho cộng đồng Nhà Hán Ngữ")}</h2>
    <p class="section-intro">${E(cfg.home_intro_text||"Nhà Hán Ngữ kết nối hoạt động, bảng tin, đăng ký tham gia, hỗ trợ cộng đồng và xác thực GCN/GXN trên cùng một ứng dụng. Giao diện công khai được giữ đơn giản, còn công cụ vận hành được đặt trong khu vực quản trị dành cho đội ngũ phụ trách.")}</p>
    <div class="feature-row"><div class="feature-card"><h3>Hoạt động</h3><p>Theo dõi chương trình, sự kiện và các đợt đăng ký đang mở.</p></div><div class="feature-card"><h3>Tra cứu</h3><p>Đối chiếu GCN/GXN trực tiếp từ dữ liệu gốc Nhà Hán Ngữ.</p></div><div class="feature-card"><h3>Cộng đồng</h3><p>Tham gia các chương trình, cộng tác và gửi phản hồi trực tuyến.</p></div></div>
  </section>
  ...
  <section class="section">
    ...
    <div class="grid">
      ...
      <div class="card"><h3>Tra cứu Giấy chứng nhận</h3><p>Đối chiếu Giấy chứng nhận bằng mã phát hành.</p></div>
...
function about(){
  ...
  app.innerHTML=`<section class="section content-prose"><div class="eyebrow">NHÀ HÁN NGỮ</div><h1>${E(c.about_title||"Nhà Hán Ngữ là gì?")}</h1><div class="lead">Một không gian học tập, kết nối và phát triển cộng đồng xoay quanh Hán ngữ.</div>${String(c.about_body||"").split("\n\n").filter(Boolean).map(x=>`<p>${E(x)}</p>`).join("")}</section>`;
}

function support(){
  const c=state.config||{};
  app.innerHTML=`<section class="section content-prose"><div class="eyebrow">HỖ TRỢ</div><h1>${E(c.support_title||"Hỗ trợ & phản hồi")}</h1>${String(c.support_body||"").split("\n\n").filter(Boolean).map(x=>`<p>${E(x)}</p>`).join("")}<div class="actions"><a class="btn" href="#form/support">Gửi yêu cầu hỗ trợ</a><a class="btn secondary" href="mailto:${E(c.receiver_email||"nhahanngu.vn@gmail.com")}">Gửi email</a></div></section>`;
}
...
async function openAccountAccess(){
  try{
    ...
    if(st?.needs_setup) return bootstrapAdmin();
    return login();
  }catch(err){
    const detail=err?.code==="DATABASE_NOT_INITIALIZED"
      ? "Cơ sở dữ liệu production chưa có đủ bảng cần thiết. Hãy chạy migrations D1 rồi tải lại trang."
      : err?.code==="DATABASE_NOT_CONFIGURED"
        ? "Worker chưa được gắn D1 binding DB. Hãy kiểm tra Cloudflare bindings rồi deploy lại."
        : "Hệ thống chưa đọc được trạng thái khởi tạo quản trị từ cơ sở dữ liệu.";
    modal(`<button onclick="closeModal()">Đóng</button>
      <h2>Không thể kiểm tra trạng thái tài khoản</h2>
      <div class="notice bad"><p>${E(detail)}</p></div>
      <p class="muted">Không thử tài khoản hoặc mật khẩu mặc định. Sau khi D1 sẵn sàng, bấm Thử lại.</p>
      <button onclick="openAccountAccess()">Thử lại</button>`);
  }
}

function bootstrapAdmin(){
  modal(`<button onclick="closeModal()">Đóng</button>
  <h2>Khởi tạo tài khoản quản trị đầu tiên</h2>
  <p class="muted">Hệ thống chưa có tài khoản quản trị. Tài khoản được tạo tại đây sẽ là Super Admin gốc.</p>
  <form id="bootstrapForm">
    <div class="field"><label>Họ và tên</label><input name="full_name" required minlength="2" autocomplete="name"></div>
    <div class="field"><label>Email đăng nhập</label><input name="email" type="email" required autocomplete="email"></div>
    <div class="field"><label>Mật khẩu</label><input name="password" type="password" required minlength="10" autocomplete="new-password"></div>
    <div class="field"><label>Nhập lại mật khẩu</label><input name="confirm_password" type="password" required minlength="10" autocomplete="new-password"></div>
    <button>Khởi tạo tài khoản</button>
  </form>`);
  bootstrapForm.onsubmit=async e=>{
    e.preventDefault(); const f=new FormData(e.target);
    if(f.get("password")!==f.get("confirm_password")) return alert("Mật khẩu nhập lại không khớp");
    try{
      const d=await api("/api/auth/bootstrap",{method:"POST",body:{full_name:f.get("full_name"),email:f.get("email"),password:f.get("password")}});
      state.user=d.user; closeModal(); location.hash="admin/dashboard";
    }catch(err){alert(err.message)}
  };
}
...
function adminNav(){
  return `<aside class="admin-nav">
    ...
    <a href="#admin/files">Thư viện tệp</a>
    <a href="#admin/settings">Cài đặt ứng dụng</a>
    ...
    ${table(["ID","Họ tên","Email","Trạng thái","Vai trò","2FA","Thao tác"],
      (d.items||[]).map(x=>`<tr>
        ...
        <td>
          ...
          <button class="secondary" onclick="toggleUser(${x.id},'${x.status==="active"?"disabled":"active"}')">${x.status==="active"?"Khóa":"Mở"}</button>
        </td>
      </tr>`)
    )}
  `);
...
async function settingsPage(){
  ...
  const values=Object.fromEntries((d.items||[]).map(x=>[x.key,x.value]));
  const txt=(k,f="")=>E(values[k]??f);
  const checked=k=>(values[k]===true||values[k]==="true")?"checked":"";
  adminPage("Cài đặt ứng dụng",`
    <div class="notice good"><b>Quản trị không góc khuất:</b> các nội dung chính của giao diện công khai, liên hệ, mạng xã hội, hero và chế độ bảo trì có thể chỉnh tại đây. Dữ liệu nghiệp vụ cũ không bị xóa.</div>
    <form id="settingsForm">
      <div class="admin-settings-grid">
        <div class="card"><h2>Nhận diện</h2>
          <div class="field"><label>Tên ứng dụng</label><input name="app_name" value="${txt("app_name","Nhà Hán Ngữ")}"></div>
        ...
        <div class="card"><h2>Liên hệ & mạng xã hội</h2>
          ...
          <div class="field"><label>Email hỗ trợ</label><input name="support_email" type="email" value="${txt("support_email")}"></div>
          <div class="field"><label>Facebook</label><input name="facebook_url" value="${txt("facebook_url","https://www.facebook.com/nhahanngu.vn")}"></div>
          <div class="field"><label>TikTok</label><input name="tiktok_url" value="${txt("tiktok_url","https://www.tiktok.com/@nhahanngu")}"></div>
          <div class="field"><label>Website</label><input name="website" value="${txt("website","https://nhahanngu.io.vn")}"></div>
        </div>
      </div>
      <div class="card" style="margin-top:18px"><h2>Nội dung Trang chủ</h2>
        <div class="field"><label>Tiêu đề giới thiệu</label><input name="home_intro_title" value="${txt("home_intro_title")}"></div>
        <div class="field"><label>Nội dung giới thiệu</label><textarea name="home_intro_text" style="min-height:170px">${txt("home_intro_text")}</textarea></div>
        <div class="field"><label>Tiêu đề CTA cộng đồng</label><input name="home_join_title" value="${txt("home_join_title")}"></div>
        <div class="field"><label>Nội dung CTA cộng đồng</label><textarea name="home_join_text" style="min-height:150px">${txt("home_join_text")}</textarea></div>
      </div>
      <div class="card" style="margin-top:18px"><h2>Trang Giới thiệu & Hỗ trợ</h2>
        <div class="field"><label>Tiêu đề Giới thiệu</label><input name="about_title" value="${txt("about_title")}"></div>
        <div class="field"><label>Nội dung Giới thiệu</label><textarea name="about_body" style="min-height:280px">${txt("about_body")}</textarea></div>
        <div class="field"><label>Tiêu đề Hỗ trợ</label><input name="support_title" value="${txt("support_title")}"></div>
        <div class="field"><label>Nội dung Hỗ trợ</label><textarea name="support_body" style="min-height:220px">${txt("support_body")}</textarea></div>
      </div>
      <div class="card" style="margin-top:18px"><h2>Vận hành</h2>
        <div class="toggle-row"><div><b>Chế độ bảo trì</b><div class="admin-note">Khi bật, người dùng công khai thấy trang bảo trì. Quản trị viên đã đăng nhập vẫn truy cập được ứng dụng.</div></div><input name="maintenance_mode" type="checkbox" ${checked("maintenance_mode")}></div>
        <div class="field"><label>Dung lượng upload tối đa (MB)</label><input name="max_upload_mb" type="number" min="1" max="100" value="${txt("max_upload_mb",10)}"></div>
      </div>
...
async function adminRoute(h){
  ...
  try{
    ...
    if(h==="admin/settings") return settingsPage();
    ...
    adminPage("Không thể tải dữ liệu",`
      <div class="notice bad">
        <b>Không thể tải phân hệ này.</b>
        <p>Vui lòng kiểm tra quyền truy cập hoặc thử lại sau.</p>
      </div>
    `);
  }
}
  ...
  try{
    ...
    app.innerHTML=`<section class="section">
      <div class="notice bad">
        ...
        <p>Ứng dụng tạm thời chưa tải được nội dung này. Vui lòng thử lại sau.</p>
      </div>
    </section>`;
  }
}
...
async function init(){
  ...
  const brandName=document.querySelector("header .brand b");
  const brandSlogan=document.querySelector("header .brand small");
  if(brandName && state.config.app_name) brandName.textContent=state.config.app_name;
  if(brandSlogan && state.config.slogan) brandSlogan.textContent=state.config.slogan;
  if(state.config.app_name) document.title=`${state.config.app_name} — Ứng dụng`;
