...
const required=[
  ...
  'migrations/0001_schema.sql','migrations/0002_seed.sql','migrations/0003_reset_auth_bootstrap.sql','migrations/0004_app_upgrade_2026.sql','migrations/0005_app_domain.sql','wrangler.jsonc'
];
let ok=true;
...
if(!wr.includes('https://app.nhahanngu.io.vn')){console.error('CONFIG FAIL: APP_URL production phải là https://app.nhahanngu.io.vn');ok=false;}
if(wr.includes('ctt.nhahanngu.io.vn')){console.error('CONFIG FAIL: còn domain production cũ ctt.nhahanngu.io.vn trong wrangler.jsonc');ok=false;}
if(ok){console.log('NHN Production validation: OK');process.exit(0)}
process.exit(1);
