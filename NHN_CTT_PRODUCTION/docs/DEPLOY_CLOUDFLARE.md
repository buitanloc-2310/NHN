# TRIỂN KHAI NHN PRODUCTION MASTER LÊN CLOUDFLARE

## A. Chuẩn bị

Mở PowerShell/Terminal tại thư mục `NHN_Production_Master`.
...
```powershell
npx wrangler r2 bucket create nhn-app-files
```
...
- `0002_seed.sql`: vai trò, modules, settings, DK-01, DK-02, privacy, 9 form, NHN, lớp mẫu, email template và Super Admin.
...
```text
Nhà Hán Ngữ <noreply@nhahanngu.io.vn>
```
...
```text
https://app.nhahanngu.io.vn/api/auth/google/callback
```
...
```text
https://app.nhahanngu.io.vn
```

Gắn custom domain/route theo cách bạn đang sử dụng trên Cloudflare. Chỉ gắn sau khi Worker URL hoạt động ổn.

## J. Khởi tạo tài khoản quản trị lần đầu

- Sau khi chạy đầy đủ migrations, mở website và bấm **Đăng nhập**.
- Nếu hệ thống chưa có tài khoản, giao diện sẽ hiện **Khởi tạo tài khoản quản trị đầu tiên**.
- Tự nhập Họ tên, Email đăng nhập và Mật khẩu (tối thiểu 10 ký tự). Không có mật khẩu mặc định trong mã nguồn.
- Tài khoản đầu tiên trở thành Super Admin gốc. Các lần sau đăng nhập bằng Email + Mật khẩu như bình thường.
- Sau đó nên vào **Bảo mật tài khoản → Thiết lập 2FA**.
...
5. Gửi 01 đăng ký lớp NHN.
6. Gửi 01 Support ticket.
7. Kiểm tra hồ sơ xuất hiện trên máy khác.
8. Kiểm tra email về `nhahanngu.vn@gmail.com`.
