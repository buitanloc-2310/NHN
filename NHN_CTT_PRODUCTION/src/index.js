...
function withCertificateCors(resp,request){
  const origin=request.headers.get("Origin")||"";
  const allowed=new Set([
    "https://skyfirst.io.vn",
    "https://www.skyfirst.io.vn",
    "http://localhost:5173",
    "http://127.0.0.1:5173"
  ]);
...
async function handle(request,env,ctx){
  ...
  if(url.pathname==="/api/health") {
    let databaseReady=false,missingTables=["settings","users"];
    if(env.DB){
      try{
        const rs=await env.DB.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('settings','users')").all();
        const names=new Set((rs.results||[]).map(x=>x.name));
        missingTables=missingTables.filter(x=>!names.has(x));
        databaseReady=missingTables.length===0;
      }catch{}
    }
    return json({
      ok:databaseReady&&!!env.FILES,app:"Nhà Hán Ngữ",production:true,time:new Date().toISOString(),
      database:!!env.DB,database_ready:databaseReady,missing_tables:missingTables,storage:!!env.FILES
    },databaseReady&&env.FILES?200:503);
  }
  ...
  const pub=await publicRoute(request,env,url); if(pub) return url.pathname==="/api/lookup/certificate" ? withCertificateCors(pub,request) : pub;
  ...
  if(request.method==="GET" && (url.pathname==="/"||url.pathname==="/index.html")){
    ...
    if(maintenance){
      const signedIn=await getAuthUser(request,env).catch(()=>null);
      if(!signedIn){
