...
export async function getAuthUser(request,env){
  ...
  // Không ghi D1 trên mọi request đọc. Điều này giúp đăng nhập vẫn nhẹ và tránh tiêu tốn row writes không cần thiết.
...
export async function authRoute(request,env,url){
  ...
  // Trạng thái khởi tạo quản trị gốc. Không chứa email/mật khẩu mặc định.
  if(p==="/api/auth/setup-status"&&request.method==="GET") {
    if(!env.DB) return json({error:"DATABASE_NOT_CONFIGURED"},503);
    const schema=await env.DB.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('settings','users')").all();
    const names=new Set((schema.results||[]).map(x=>x.name));
    const missing=["settings","users"].filter(x=>!names.has(x));
    if(missing.length) return json({error:"DATABASE_NOT_INITIALIZED",missing_tables:missing},503);
    const rootId=await env.DB.prepare("SELECT value_json FROM settings WHERE key='super_admin_user_id'").first();
    const count=await env.DB.prepare("SELECT COUNT(*) AS n FROM users").first();
    return json({needs_setup:!rootId && Number(count?.n||0)===0,database_ready:true});
  }

  // Chỉ tài khoản đầu tiên của một CSDL chưa khởi tạo mới được trở thành Super Admin.
  if(p==="/api/auth/bootstrap"&&request.method==="POST") {
    let userId=null,stage="rate_limit";
    try {
      const rl=await rateLimit(env,`bootstrap:${await ipHash(request)}`,5,600);
      if(!rl.ok) return json({error:"RATE_LIMIT"},429);

      stage="precheck";
      const existingRoot=await env.DB.prepare("SELECT value_json FROM settings WHERE key='super_admin_user_id'").first();
      const count=await env.DB.prepare("SELECT COUNT(*) AS n FROM users").first();
      if(existingRoot || Number(count?.n||0)>0) return json({error:"SETUP_ALREADY_COMPLETED"},409);
      ...
      // 100k vòng để phù hợp giới hạn CPU của Workers và đồng bộ với luồng đổi mật khẩu.
      stage="password_hash";
      const salt=newSalt(),hash=await pbkdf2(password,salt,100000);

      stage="create_user";
      const res=await env.DB.prepare("INSERT INTO users(email,full_name,password_salt,password_hash,password_iterations,status,email_verified,must_change_password) VALUES(?,?,?,?,100000,'active',1,0)")
        .bind(email,name,salt,hash).run();
      userId=Number(res.meta.last_row_id);
      if(!Number.isFinite(userId)||userId<=0) throw new Error("USER_ID_NOT_RETURNED");

      stage="grant_super_admin";
      await env.DB.batch([
        env.DB.prepare("INSERT INTO user_roles(user_id,role_id,scope_unit_code,granted_by) VALUES(?,'super_admin','',?)").bind(userId,userId),
        env.DB.prepare("INSERT OR REPLACE INTO settings(key,value_json,updated_by) VALUES('super_admin_user_id',?,?)").bind(JSON.stringify(userId),userId),
        env.DB.prepare("INSERT OR REPLACE INTO settings(key,value_json,updated_by) VALUES('super_admin_email',?,?)").bind(JSON.stringify(email),userId)
      ]);
      ...
      return json({ok:true,user},201,{"set-cookie":sessionCookie(session.token)});
    } catch(err) {
      console.error("AUTH_BOOTSTRAP_FAILED",{stage,userId,message:String(err?.message||err),stack:err?.stack||null});

      // Nếu lỗi xảy ra sau khi INSERT user, dọn trạng thái tạo dở để người quản trị có thể thử lại.
      if(userId){
        try {
          await env.DB.batch([
            env.DB.prepare("DELETE FROM sessions WHERE user_id=?").bind(userId),
            env.DB.prepare("DELETE FROM user_roles WHERE user_id=?").bind(userId),
            env.DB.prepare("DELETE FROM settings WHERE key IN ('super_admin_user_id','super_admin_email')"),
            env.DB.prepare("DELETE FROM users WHERE id=?").bind(userId)
          ]);
        } catch(cleanupErr){
          console.error("AUTH_BOOTSTRAP_CLEANUP_FAILED",{userId,message:String(cleanupErr?.message||cleanupErr)});
        }
      }
      return json({error:"BOOTSTRAP_FAILED",stage},500);
    }
  }
  ...
  if(p==="/api/me/2fa/setup"&&request.method==="POST"){
    ...
    const issuer=encodeURIComponent("Nhà Hán Ngữ"),label=encodeURIComponent(user.email);
...
export async function createInvitedAccount(request,env,actor,body){
  ...
  let temp="NHN!"; for(let i=0;i<rnd.length;i++) temp+=chars[rnd[i]%chars.length];
