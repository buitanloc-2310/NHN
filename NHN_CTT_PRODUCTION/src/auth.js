...
export async function authRoute(request,env,url){
  ...
  if(p==="/api/auth/bootstrap"&&request.method==="POST") {
    const rl=await rateLimit(env,`bootstrap:${await ipHash(request)}`,5,600); if(!rl.ok) return json({error:"RATE_LIMIT"},429);
    ...
    const salt=newSalt(),hash=await pbkdf2(password,salt,210000);
    const res=await env.DB.prepare("INSERT INTO users(email,full_name,password_salt,password_hash,password_iterations,status,email_verified,must_change_password) VALUES(?,?,?,?,210000,'active',1,0)")
    ...
    const session=await createSession(request,env,userId),user=await getUserById(env,userId);
  ...
  if(p==="/api/auth/register"&&request.method==="POST"){
    ...
    const salt=newSalt(),hash=await pbkdf2(password,salt,210000);
    const res=await env.DB.prepare("INSERT INTO users(email,full_name,password_salt,password_hash,password_iterations,status,email_verified,must_change_password) VALUES(?,?,?,?,210000,'active',0,0)")
      .bind(email,name,salt,hash).run();
