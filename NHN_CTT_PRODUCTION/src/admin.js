...
async function rootSuperAdminId(env){
  const id=Number(await setting(env,"super_admin_user_id",0));
...
const GENERIC = {
  ...
  classes:{
    ...
    defaults:{unit_code:"NHN",status:"Đang mở đăng ký",schedule_json:"{}",data_json:"{}"}
  },
  events:{
    permission:"event.manage",table:"events",id:"id",
    fields:["unit_code","title","start_at","end_at","status","capacity","data_json"],
    defaults:{unit_code:"NHN",status:"Sắp diễn ra",data_json:"{}"}
  },
  units:{
...
export async function adminRoute(request,env,url){
  ...
  if(convertMatch&&request.method==="POST"){
    ...
    const res=await env.DB.prepare("INSERT INTO people(user_id,source_submission_code,full_name,email,phone,unit_code,position,status,joined_at,profile_json) VALUES(?,?,?,?,?,?,?,'Đang hoạt động',date('now'),?)")
      .bind(linked?.id||null,code,s.full_name||"",s.email||"",a.phone||"",a.area||a.volunteer_type||"NHN",a.position||s.form_id,JSON.stringify({teaching_scope:a.teaching_scope||null})).run();
  ...
  if(rolesMatch&&request.method==="PUT"){
    ...
    if(targetId===(await rootSuperAdminId(env)) && Number(user.id)!==targetId)
    ...
    for(const old of existing){
      if(old.role_id==="super_admin"&&targetId===(await rootSuperAdminId(env))&&!roles.some(r=>(r.role_id||r)==="super_admin"))
        return json({error:"ROOT_SUPER_ADMIN_CANNOT_BE_REMOVED"},403);
    }
    if(targetId===(await rootSuperAdminId(env))) await env.DB.prepare("DELETE FROM user_roles WHERE user_id=? AND role_id!='super_admin'").bind(targetId).run(); else await env.DB.prepare("DELETE FROM user_roles WHERE user_id=?").bind(targetId).run();
    for(const r0 of roles){
      ...
      if(role==="super_admin"&&targetId!==(await rootSuperAdminId(env))&&!(user.roles||[]).some(r=>r.role_id==="super_admin")) continue;
  ...
  if(userResetMatch&&request.method==="POST"){
    ...
    if(!target)return json({error:"NOT_FOUND"},404);
    if(targetId===(await rootSuperAdminId(env)))return json({error:"ROOT_SUPER_ADMIN_PROTECTED"},403);
    const chars="ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789!@#%",rnd=new Uint8Array(16);crypto.getRandomValues(rnd);
    let temp="NHN!";for(const b of rnd)temp+=chars[b%chars.length];
  ...
  if(userStatusMatch&&request.method==="PATCH"){
    ...
    if(!target)return json({error:"NOT_FOUND"},404);
    if(targetId===(await rootSuperAdminId(env))&&status!=="active") return json({error:"ROOT_SUPER_ADMIN_PROTECTED"},403);
    await env.DB.batch([
  ...
  if(p==="/api/admin/forms"&&request.method==="POST"){
    ...
    await env.DB.prepare("INSERT INTO forms(id,name,prefix,description,audience,min_age,enabled,recipient_email,version,config_json,updated_by) VALUES(?,?,?,?,?,?,1,?,1,?,?)")
      .bind(idForm,body.name,body.prefix,body.description||"",body.audience||"public",body.min_age??null,body.recipient_email||"nhahanngu.vn@gmail.com",JSON.stringify(body.config),user.id).run();
  ...
  if(certMatch&&request.method==="PATCH"){
    ...
    if(body.action==="issue"){
      ...
      const n=await nextCounter(env,`certificate:${kind}:${year}`),code=`${String(n).padStart(3,"0")}/${kind}-NHN/${year}`;
  ...
  if(p==="/api/admin/export/submissions.csv"&&request.method==="GET"){
    ...
    return new Response("\ufeff"+rows.join("\n"),{headers:{"content-type":"text/csv; charset=utf-8","content-disposition":"attachment; filename=NHN_submissions.csv"}});
  }
  ...
  if(p==="/api/admin/upload"&&request.method==="POST"){
    ...
    const allowed=new Set(["image/jpeg","image/png","image/webp","image/gif","application/pdf","text/plain","text/csv","application/zip","audio/mpeg","audio/wav","video/mp4","application/vnd.openxmlformats-officedocument.wordprocessingml.document","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","application/vnd.openxmlformats-officedocument.presentationml.presentation"]);
